/**
 * NodeList.jsx — browse all nodes with filtering by type and search.
 */
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex gap-3 px-6 py-3 border-b border-gray-100 bg-white shrink-0">
        <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}
          ...
          {types.map((t) => (
            <option key={t.id} value={t.node_type_identifier}>{t.node_type_name}</option>
          ))}
        </select>
        <input
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                {['Type', 'Identifier', 'Name', 'Status'].map((h) => (
              ...
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 text-xs font-mono text-gray-600">{row.node_identifier}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-900">{row.node_name}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-500">{row.created_on?.slice(0, 10)}</td>
                  </tr>
                )
              })}
              {rows.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-400">No nodes found</td></tr>
              )}
            </tbody>
          </table>
        )}
